 ...
 * @Type Customer
 * @Date 2018. 3. 28.
 * @Description customer라는 id가 붙은 bean이 생성된다.
 */
@Named // Named = Component와 같은 역할.
public class Customer {
	
	@Inject // this Annotation inject orderManager Object.
	private OrderManager orderManager;
	
	private static Wallet moneyC;
	// Constructor
	public Customer() {

	}
	// Setter Wallet 관련.
	public void setMoneyC(Wallet moneyC) {
		Customer.moneyC = moneyC;
	}
...
//		manager.order();
*/	
		// ApplicationContext ctx = new ClassPathXmlApplicationContext("/spring/car-config.xml");
		// WAS가 기동할 때 처리한다.
		ApplicationContext ctx = new ClassPathXmlApplicationContext("/spring/car-config-annotation.xml");
		
		
		
		// Bean : Annotation을 사용하여 DI를 통해 객체를 얻는다.
		OrderManager orderManager = ctx.getBean("orderManager",OrderManager.class);
		// 
		String myCar = orderManager.order(moneyC);
		
		moneyC.setMoney(0);// 돈을 다 썼음.
		...
		customer.orderManager.order(moneyC);
		System.out.println("************TEST:IsSINGLETON?************");
		System.out.println("object1: "+(orderManager).toString().substring(29,51));
		System.out.println("object2: "+(customer.orderManager).toString().substring(29,51));
		System.out.println("object3: "+(ctx.getBean("orderManager", OrderManager.class)).toString().substring(29,51));
		System.out.println("************RESULT: "+(orderManager == customer.orderManager ? 
									  orderManager == ctx.getBean("orderManager",OrderManager.class) ?
											  "isTrue************" : "isFalse************" : "isFalse************"));
		System.out.println("END=====================================================================");
		
		((AbstractApplicationContext) ctx).close();// 다 쓴 ctx 닫기. 자원 누수 막기 위함.
	}// main()
